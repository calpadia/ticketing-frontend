...
- **[26 Agustus 2026] Centralized Attachments**: Menggabungkan file lampiran dari form registrasi tiket awal dan lampiran dari riwayat *chat* ke dalam satu kotak *Attachments* terpusat di `TicketDetail.vue`.
- **[26 Agustus 2026] Rich Notification Dropdown**: Merombak komponen notifikasi di `Layout.vue` untuk menampilkan rincian pesan masuk. Menambahkan fungsi navigasi instan untuk membuka spesifik tiket melalui notifikasi dengan paksaan re-render halaman (`:key="$route.fullPath"`).
- **[26 Agustus 2026] Strict Input Filters**: Mengamankan semua kolom nomor telepon di seluruh aplikasi (`ClientOnboarding.vue`, `Users.vue`, `Clients.vue`, `Register.vue`) dengan 3 lapis proteksi ketat (`inputmode="numeric"`, `pattern="[0-9]*"`, dan *Real-time Regex Regex replace(/\D/g, '')*) agar kebal terhadap input huruf.

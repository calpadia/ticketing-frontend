  ...
  <div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
      <table class="w-full text-sm">
        <thead class="bg-gray-50"><tr class="text-left text-gray-600"><th class="px-6 py-3 font-medium">Name</th><th class="px-6 py-3 font-medium">Email</th><th class="px-6 py-3 font-medium">Role</th><th class="px-6 py-3 font-medium">Actions</th></tr></thead>
        <tbody class="divide-y divide-gray-100">
          <tr v-if="filtered.length === 0"><td colspan="4" class="text-center py-8 text-gray-500">No users found.</td></tr>
          <tr v-for="u in filtered" :key="u.id" class="hover:bg-gray-50">
            <td class="px-6 py-3"><div class="flex items-center gap-3"><div class="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center text-blue-700 font-medium text-sm">{{ u.name.charAt(0).toUpperCase() }}</div><span class="font-medium text-gray-900">{{ u.name }}</span></div></td>
            ...
            <td class="px-6 py-3"><div class="flex gap-2"><button @click="startEdit(u)" class="p-1.5 rounded-lg hover:bg-blue-50 text-blue-600"><Pencil class="w-4 h-4" /></button><button @click="handleDelete(u.id)" class="p-1.5 rounded-lg hover:bg-red-50 text-red-600"><Trash2 class="w-4 h-4" /></button></div></td>
          </tr>
        </tbody>
      </table>
    </div>
...
const filtered = computed(() => users.value.filter(u => u.name.toLowerCase().includes(search.value.toLowerCase()) || u.email.toLowerCase().includes(search.value.toLowerCase())))

onMounted(async () => { try { users.value = (await getUsers()).data } catch { users.value = [] } })

function toggleForm() { showForm.value = !showForm.value; editingId.value = null; Object.assign(form, { name: '', email: '', password: '', role: 'USER' }) }
function startEdit(u) { editingId.value = u.id; Object.assign(form, { name: u.name, email: u.email, password: '', role: u.role }); showForm.value = true }
...
async function handleSubmit() {
  ...
  showForm.value = false; editingId.value = null; Object.assign(form, { name: '', email: '', password: '', role: 'USER' })
}
